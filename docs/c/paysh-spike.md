...
| 단일 endpoint 결제 challenge (createChallenge) | **VERIFIED (sandbox)** | `pay --sandbox curl https://debugger.pay.sh/mpp/quote/AAPL` → 402 challenge를 CLI가 투명 처리 후 응답 수신 (조사 로그 #2) | x402 흐름 실동작. challenge *발급*(서버측)은 우리 gateway 몫 — 미검증 |
...
docs/06 원칙대로 **fallback 확정**: 세션 usage cap 집행은 pay.sh가 아니라 gateway(사전검사) + bond 프로그램(위반 시 제한 정산)이 담당한다. pay.sh는 (a) per-call 402 결제 rail — sandbox 실검증 완료, (b) MPP subscription delegation — 표면 확인, E2E는 Plan PDA(provider측) 필요로 데모 범위 밖. 피치 문구: "pay.sh x402로 per-call 과금, 세션 상한은 BotBond bond가 담보" — pay.sh가 세션 cap을 제공한다고 말하지 않는다.
