...
The sponsored browser runner creates fresh Solana-devnet bond-open and refund or bounded-settlement transactions. It has no per-IP wait; only a daily budget and single-concurrency limit protect the sponsor wallet. It never exposes a scoped token or signer secret.
