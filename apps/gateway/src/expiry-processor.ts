...
export async function processExpiredReservations(dependencies: ExpiryProcessorDependencies): Promise<ExpiryResult[]> {
  ...
  for (const session of sessions.filter((entry) => entry.state === "ACTIVE" && entry.policy.bondedActions.length > 0)) {
...
async function expireOne(dependencies: ExpiryProcessorDependencies, session: SessionRecord, reservation: ReservationRecord): Promise<ExpiryResult> {
  ...
  const evidence = createSettlementEvidence({ sessionId: session.sessionId, policyHash: session.policyHash, reservationId: reservation.reservationId, outcome: "EXPIRED_RESERVATION", usageChargedAtomic, penaltyAtomic, bondRefundedAtomic, nonce: `expiry:${reservation.reservationId}`, issuedAt: dependencies.clock.now().toISOString(), authority: dependencies.settlementAuthority ?? "botbond-expiry-worker-local" }, dependencies.settlementSigningSecret ?? process.env.SETTLEMENT_SIGNING_SECRET ?? "fake-local-settlement-secret");
  const settlement = await dependencies.bond.requestExpiredReservationSettlement({
    sessionId: session.sessionId,
  ...
  if (!finalized.changed) return { reservationId: reservation.reservationId, status: "SKIPPED", reason: "ALREADY_FINALIZED" };
  ...
  const expiredSession = await dependencies.repository.transitionSession(session.sessionId, "ACTIVE", "EXPIRED");
