...
PAY_GATE_SERVICE_ACCOUNT="botbond-pay-gate@${PROJECT_ID}.iam.gserviceaccount.com"
...
for account in botbond-intent botbond-gateway botbond-web botbond-pay-gate; do
...
rtk gcloud projects add-iam-policy-binding "$PROJECT_ID" --member "serviceAccount:${PAY_GATE_SERVICE_ACCOUNT}" --role roles/secretmanager.secretAccessor

for secret in botbond-devnet-wallet botbond-evidence-secret botbond-payment-secret botbond-mpp-secret; do
...
rtk gcloud run deploy botbond-gateway \
  ...
  --allow-unauthenticated \
  --timeout 300 \
  --set-env-vars "ADAPTER_MODE=solana,REPOSITORY_MODE=firestore,FIRESTORE_NAMESPACE=botbond,GOOGLE_CLOUD_PROJECT=${PROJECT_ID},INTENT_COMPILER_URL=${INTENT_URL},ANCHOR_PROVIDER_URL=https://api.devnet.solana.com,ANCHOR_WALLET=/secrets/botbond-devnet.json,SOLANA_CLUSTER=devnet,BOTBOND_PROGRAM_ID=EG9rKPV69v3WNX7aVchAPonMtKPp6yML7jZwDjMKaRKR,SETTLEMENT_AUTHORITY=GMrS1AR2MmHvW9cDmQJ4RApQz6iTS17srfb8bwucJQa6,PUBLIC_DEMO_ENABLED=true,PUBLIC_DEMO_MINT=6iPkutkbMLoMc4bTqUxB8m1d2dtS5RQR2o5ZikW7sVPw,PUBLIC_DEMO_MERCHANT=zdHstotkcGBUSD6BA8QR4osRTMQ7UnzHa3HpvjZYfts,PUBLIC_DEMO_DAILY_LIMIT=30,PUBLIC_DEMO_COOLDOWN_MS=600000,PUBLIC_DEMO_LEASE_MS=120000,PAY_GATE_URL=https://botbond-pay-gate-752329931962.us-central1.run.app" \
...
rtk gcloud run deploy botbond-pay-gate \
  --image "${REPOSITORY}/pay-gate:${IMAGE_TAG}" \
  --region "$REGION" \
  --service-account "$PAY_GATE_SERVICE_ACCOUNT" \
  --allow-unauthenticated \
  --set-secrets "MPP_SECRET_KEY=botbond-mpp-secret:latest"
PAY_GATE_URL="$(rtk gcloud run services describe botbond-pay-gate --region "$REGION" --format='value(status.url)')"
